...
| `worker.shared.session.enabled`           | `false` | Enables the shared CQL session cache for tasks in the same JVM when their Scylla connection settings match. The default `false` preserves the legacy behavior where each task creates its own dedicated session. |
| `worker.max.retries`                      | `20`    | Maximum number of CDC worker attempts before failing the task when `worker.shared.session.enabled=true`. This is a total attempt count, not a retry count: `1` means one run with no retries on failure, `-1` means unlimited attempts. |
